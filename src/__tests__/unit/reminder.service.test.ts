...
describe("ReminderServiceV2 - Unit Tests", () => {
  ...
  beforeEach(() => {
    ...
    mockRepository =
      ReminderRepository.prototype as jest.Mocked<ReminderRepository>;
  ...
  describe("create", () => {
    ...
    it("should create reminder without Camunda when service unavailable", async () => {
      ...
      mockRepository.create.mockResolvedValue(mockReminder);
      mockCamundaService.startReminderProcess.mockRejectedValue(
        new Error("Camunda unavailable"),
      );
  ...
  describe("getById", () => {
    ...
    it("should enforce ownership when userId provided", async () => {
      ...
      await expect(service.getById("123", "different-user")).rejects.toThrow(
        NotFoundError,
      );
    });
  });
  ...
  describe("list", () => {
    ...
    it("should reject invalid pagination parameters", async () => {
      ...
      await expect(service.list({ limit: 150 })).rejects.toThrow(
        ValidationError,
      );
  ...
  describe("update", () => {
    ...
    it("should enforce ownership when userId provided", async () => {
      ...
      await expect(
        service.update("123", { title: "Updated" }, "different-user"),
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe("processDueReminders", () => {
    ...
    it("should handle errors gracefully and continue processing", async () => {
      ...
      mockRepository.findDueReminders.mockResolvedValue(dueReminders);
      mockRepository.markAsNotified.mockRejectedValue(
      ...
      expect(mockEventLogger.log).not.toHaveBeenCalledWith(
        "NOTIFICATION_SENT",
        expect.any(Object),
      );
    });
...
describe("ReminderValidator - Business Rules", () => {
  ...
  describe("canTransitionTo", () => {
    it("should allow valid transitions", () => {
      expect(
        ReminderValidator.canTransitionTo(
